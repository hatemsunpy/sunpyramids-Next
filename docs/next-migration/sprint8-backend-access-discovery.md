...
| ---- | ---------------- | ------------------------ | ----------- | ---------- | ----------- | -------------- | ------ | ----- |
| Confirmed production frontend URL (no staging found) | Partial | `.env` has `APP_FRONTEND_URL=https://sunpyramidstours.com`; no staging frontend found. | `.env`, `app/Http/Controllers/Api/AuthController.php` | High | Yes for public production URL | Yes | Partial | This confirms production frontend for social redirects, not staging. |
...
| Local backend/API URL defaults only (no staging found) | Partial | `.env` and `.env.example` default `APP_URL=http://127.0.0.1:8000`; API prefix is `/api`. | `.env`, `.env.example`, `RouteServiceProvider.php` | High | Yes | Yes | Partial | No staging backend URL found; public backend domain remains owner-provided/project rule. |
