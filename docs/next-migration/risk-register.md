...
| Guest cart is keyed by client public IP, not a per-user cookie/token | Cart/checkout UX | Medium | `cart/tours/append` returns no `Set-Cookie`; `cart/list` serves items with zero cookies/auth; identity is the caller's public IP server-side. Same behavior on live Nuxt. Sharing a public IP (office/hotel NAT) shares the cart; an IP change loses it. | Parity with live site — acceptable for cutover. If per-user guest cart is required, backend owner adds a guest cart token (see `guest-cart-session-investigation.md`). |
