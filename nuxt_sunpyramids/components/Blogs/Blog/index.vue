<template>
  <section v-if="data"
    ...
    <div class="2xl:col-span-5 lg:col-span-5  col-span-8 hidden lg:block">
      <h2 class="py-8 text-primary font-medium text-[34px]">{{ props.title }}</h2>
    ...
    <div class="2xl:col-span-2 lg:col-span-3  col-span-8 lg:sticky h-fit top-20  overflow-hidden">
      <div class="bg-[#F9FAFB] rounded-[2rem]  py-4 mb-4  overflow-hidden ">
        <h6 class="text-xl lg:px-8 px-2 mb-6 font-medium text-textLight">
          {{ $t("labels.blog.onThisPage") }}
        </h6>
        <div class="lg:max-h-[280px] lg:px-8 px-2 lg:overflow-y-auto">
      ...
      <div class="rounded-[2rem] bg-[#f9fafb] hidden lg:block relative overflow-hidden">
        ...
        <div class="py-1 flex justify-center items-center px-4 gap-4 bg-[#F9FAFB]">
          <UIButton :text="$t(`labels.exploreTours`)" :classes="[
            'border border-primary w-full !justify-center hover:bg-primary  border-[1px]  !px-0 !py-3 !font-meduim hover:text-white',
          ]" @click="router.push(localePath('/trips'))" />

          <UIButton @click="router.push(localePath('/make-your-trip'))" :loading="isLoading" :classes="[
            'py-3 !px-0 w-full gap-2 border !justify-center bg-primary border-primary border-[1px] text-base font- hover:bg-[#143485] text-white font-meduim hover:text-white',
...
watch(() => props.data, (value) => {
  parseStrongParagraphsAsHeadings(value)
},
  { immediate: true }
)
...
function updateH2Headings(data) {
  const result = [];
  let lastH2 = null;
  data = data.filter(item => item.title.includes('<h2') || item.title.includes('<h3'))

  data.forEach(item => {
    if (item.title.includes('<h2')) {
      ...
      result.push(lastH2);
    } else if (item.title.includes('<h3') && lastH2) {
