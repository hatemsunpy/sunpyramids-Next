<template>
  <section v-if="relatedBlogs.length" class="xl:px-20 px-4 pt-10 pb-14 w-full">
...
relatedBlogs.value = await getData(`blogs?page_limit=5&order_by=display_order,asc&categories.slug=${route.params.slug}`).then((res) => {
