  ...
  <VeeForm :validation-schema="schema" v-slot="{ values, errors }" :initial-values="props.initialValues.personalInfo"
    ...
    <div class="grid grid-cols-2 gap-5 pb-24 md:pb-0">
      <div class="w-full col-span-2 grid grid-cols-2 gap-5 gap-y-8">
        ...
        <div class="flex col-span-2 gap-5 lg:flex-row flex-col">
          <UISelect @click="onClickCountries" :isRequired="true" :items="nationalities" title="name" value="id"
            name="nationality" :placeholder="$t('inputLabels.nationalityPlaceholder')"
        ...
        <div class="col-span-2 grid gap- lg:grid-cols-3 gap-5">
...
const schema = yup.object().shape({
  fullName: yup.string().required(t("errors.isRequired", { name: t("inputLabels.fullName") })).matches(
    /^[^\d]*$/,
    t("errors.invalidCharacters", { name: t("inputLabels.fullName") }) // Customize this message
  ),
  email: yup.string().email(t("errors.email", { name: t("inputLabels.email") }))
