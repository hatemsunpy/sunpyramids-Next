  ...
  <section class="2xl:px-20 px-4 xl:px-10 xl:py-14 pt-5  bg-[#FFFFFF] flex gap-6">
    ...
    <div class="w-full">
      ...
      <div class="flex justify-between">
        <div class="w-full scroll-container flex items-center gap-4">
          ...
          <button v-for="item in filterBtn"
            class="px-6 py-2 bg-[#F7F7F7] text-sm hover:bg-[#eeeeee] min-w-fit rounded-full flex gap-2" :class="[
            ...
            <img v-if="selecterBtnsFilter.some((btn) => btn == item)" src="../../../public/icons/close-circle.svg"
              alt="" class="-me-3 w-5" />
          </button>
        </div>
      ...
      <div v-if="!isLoading && tours" class="w-full grid gap-x-5 gap-y-8 my-8 md:mb-[4.5rem] openFilterGrids">
...
const selecterBtnsFilter = ref(["All"])
...
// const catogries = [{ name: "Festival & Events", number: 99 }]
...
const getTours = async (values) => {
  ...
  if (selecterBtnsFilter.value.includes("All")) {
    params["categories.slug[]"] = ["night-tours", "one-day-tours", "half-day-tour", "layover"]
    params = [...params, "&categories.slug%5B%5D=night-tours", "&categories.slug%5B%5D=one-day-tours", "&categories.slug%5B%5D=half-day-tour", "&categories.slug%5B%5D=layover"]
  } else {
    if (selecterBtnsFilter.value.includes("Night Tours")) params = [...params, "&categories.slug%5B%5D=night-tours"]
    if (selecterBtnsFilter.value.includes("Day Tours")) params = [...params, "&categories.slug%5B%5D=one-day-tours"]
    if (selecterBtnsFilter.value.includes("Half Day Tour")) params = [...params, "&categories.slug%5B%5D=half-day-tour"]
    if (selecterBtnsFilter.value.includes("Layover")) params = [...params, "&categories.slug%5B%5D=layover"]
  }
  if (!order.value) {
