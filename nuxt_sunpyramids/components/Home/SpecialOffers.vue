      ...
      <div class="flex lg:justify-center justify-between lg:items-center items-start">
        <div>
          <div class="flex lg:justify-center  justify-start items-center gap-2">
            ...
            <h3 class="lg:text-5xl md:text-3xl text-xl text-start lg:text-center font-bold">
              {{ $t("labels.SpecialOfferForYou") }}
            </h3>
          </div>
    ...
    <div class="w-full xl:px-20 mb-10">
      <div v-if="data.length">
        <div class="lg:grid hidden w-full  2xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-x-5 gap-y-8">
          <SharedTourCard v-for="tour in data" :item="tour" :key="tour" :showTimer="true" />
        </div>
        ...
        <div class="block lg:hidden">
          <swiper :modules="modules" :autoplay="{
            ...
            <swiper-slide class="w-[15rem] group cursor-pointer h-[15rem] rounded-2xl overflow-hidden"
              ...
              <SharedTourCard :item="tour" :showTimer="true" />
            </swiper-slide>
      ...
      <div class="mt-8 lg:block hidden">
        <SharedSeeMore path="/trips?main=special-offers" />
      </div>
    </div>
  </section>
</template>
...
const url = `tours/home?page=1&page_limit=4&order_by=display_order,asc&categories.id=53`
