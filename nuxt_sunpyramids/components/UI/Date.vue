<template>
  <client-only>
    <div class="flex flex-col gap-3 w-full">
      ...
      <div @click="DatePickerInput.toggleMenu()"
        class="flex items-center gap-4 relative px-8 cursor-pointer py-2 w-full border-dark border rounded-[3.5rem] inputBase">
        <div class="flex flex-col gap-2 w-full">
          <VueDatePicker auto-position="top" ref="DatePickerInput" :disabled="props.disabled"
            ...
            :auto-apply="true" :name="props.name" v-model="fieldValue" :placeholder="props.placeholder"
            :month-picker="props.isMonth" :enable-time-picker="props.isTime" @closed="meta.touched = true"
            class="font-medium ms-[1px]" />
        </div>
        <NuxtIcon v-if="props.icon" :name="props.icon"
          class="min-w-fit text-xl w-5 h-5 text-textLight cursor-pointer" />
...
const disableExceptSpecificDates = (date) => {
  ...
  return props.condition ? props.condition(day) : false;
}
</script>
